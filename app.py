...
@st.cache_data
def load_model():
    with open('model/kidney_disease_model', 'rb') as file:
...
options = st.sidebar.selectbox('Select a page:', 
                               ['Home', 'Prediction', 'Code', 'About'])

if options == 'Home':  # Home page
    st.title('Chronic Kidney Disease Prediction Project')
    st.write('Welcome to the Chronic Kidney Disease Prediction web app! This app leverages machine learning to predict whether a patient has chronic kidney disease (CKD) based on various health parameters.')
    st.write('The model used in this app is trained on a dataset containing different features like blood pressure, age, albumin levels, and more.')
    st.write('### Key Features:')
    st.write('- Predicts the likelihood of chronic kidney disease based on input health parameters.')
    st.write('- Built with a Support Vector Machine (SVM) model for high accuracy.')
    st.write('- Simple user interface for easy input of health data.')
    st.write('### Technologies Used:')
    st.write('- Python, Streamlit for the web app interface.')
    st.write('- Machine Learning (SVM) for the prediction model.')
    st.write('- Pandas for data processing.')
    st.write('The web app is open-source. You can view the code and the dataset used in this web app from the GitHub repository below.')
    

elif options == 'Prediction':  # Prediction page
...
elif options == 'Code':  # Code page
    st.header('Code')
    # Add a button to download the Jupyter notebook (.ipynb) file
    notebook_path = 'model/model.ipynb'
    ...
    st.title('About')
    st.write('This web app is created to predict chronic kidney disease using the data from the UCI Machine Learning Repository.')
    ...
    st.write('The dataset is used to build a machine learning model to predict chronic kidney disease based on the input parameters.')
    st.write('--'*50)

    st.write('The web app is open-source. You can view the code and the dataset used in this web app from the GitHub repository below.')
    st.write('[GitHub Repository](XGBoost-Driven Insights: Enhancing Chronic Kidney Disease Detection)')
    st.write('--'*50)
